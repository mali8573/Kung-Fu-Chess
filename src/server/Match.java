...
public class Match {
    ...
    private volatile WebSocketSession whiteConn;
    private volatile WebSocketSession blackConn;
    ...
    private final List<WebSocketSession> spectators = new CopyOnWriteArrayList<>();
    ...
    public Match(AccountStore accountStore,
                 WebSocketSession whiteConn, String whiteName, int whiteElo,
                 WebSocketSession blackConn, String blackName, int blackElo) {
    ...
    public WebSocketSession blackConnection() { return blackConn; }
     ...
     *  the right square. */
    public void handleMessage(WebSocketSession connection, String[] parts) {
    ...
    public void onPlayerDisconnected(WebSocketSession connection) {
        synchronized (lock) {
    ...
    public boolean reconnect(WebSocketSession newConnection) {
     ...
     *  100ms tick. */
    public void addSpectator(WebSocketSession connection) {
    ...
    public void removeSpectator(WebSocketSession connection) {
    ...
    private void broadcastState() {
        ...
        for (WebSocketSession spectator : spectators) trySend(spectator, state);
    }
    ...
    /** Best-effort send: a gone/closed session must never crash this Match's tick thread (it
     *  needs to keep ticking so the OTHER side still gets its disconnect countdown / game-over).
     *  Spring's WebSocketSession throws IllegalStateException (not IOException) for "already
     *  closed", and ConcurrentWebSocketSessionDecorator can throw its own runtime exceptions if
     *  a send exceeds the configured time/buffer limit - all are just as harmless to ignore here
     *  as the IOException the old hand-rolled WebSocketConnection.sendText used to throw. */
